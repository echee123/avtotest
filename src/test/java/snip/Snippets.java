...
public class Snippets {
    ...
    void browser_command_examples() {
        ...
        Selenide.refresh(); // кнопка обновления страницы

        Selenide.clearBrowserCookies();//очищение куки
        Selenide.clearBrowserLocalStorage();//очищение истории
        executeJavaScript("sessionStorage.clear();"); // no Selenide command for this yet

        Selenide.confirm(); // OK в alert окне
        Selenide.dismiss(); // Cancel в alert окне

        Selenide.closeWindow(); // закрыть активный браузер
        Selenide.closeWebDriver(); // закрыть все вкладки браузера

        Selenide.switchTo().frame("new");
        Selenide.switchTo().defaultContent(); // return from frame back to the main DOM

        Selenide.switchTo().window("The Internet");//переход между окнами браузера

        var cookie = new Cookie("foo", "bar"); //добавить куки в барузер (всегда обновлять после страницу)
    ...
    void selectors_examples() {
        $("div").click(); // поиск элемента
        element("div").click();

        $("div", 2).click(); // ищет второе вхождение элемента

        $x("//h1/div").click();
        $(byXpath("//h1/div")).click();

        $(byText("full text")).click();// поиск по тексту
        $(withText("ull tex")).click();// поиск по частичному сопвпадению

        $(byTagAndText("div", "full text"));//элемент+текст
        $(withTagAndText("div", "ull text"));//элемент+текст
    ...
    void actions_examples() {
        ...
        $("").setValue("text");//записать текст
        $("").append("text");//добивать в уже написанный текст
        $("").clear();//очистить поле
        $("").setValue(""); // clear
    ...
    void conditions_examples() {
        ...
        $("").should(matchText("[0-9]abc$"));//проверка формата

        $("").shouldHave(cssClass("red"));//содержит ли класс хоть 1 элемент
        $("").shouldHave(cssValue("font-size", "12"));//проверить соответсвие
